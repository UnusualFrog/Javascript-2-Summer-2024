// Create cargo status and manifest elements
...
const processCargo = () => {
    ...
    if (transportID == "") {
        ...
        let statusRow = `<tr><td class=\"transportID\">${transportID}</td><td class=\"description\">${description}</td><td class=\"weight\">${cargoWeight}</td><td class=\"status\">${boxCarId}</td></tr>`;
...
// Attach event listeners to process and reset buttons on DOM load
$("document").ready(() => {
    console.log("Ready!");

    // Set current weight to empty weight on DOM load
    $("#CurrentWeight").val(parseFloat($("#EmptyWeight").val()));

    // Attach listeners to functional buttons
